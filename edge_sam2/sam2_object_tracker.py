...
from edge_sam2.modeling.sam2_base import SAM2Base, NO_OBJ_SCORE
from edge_sam2.modeling.sam2_utils import get_1d_sine_pe
from edge_sam2.utils.kalman_filter import KalmanFilter
...
class SAM2ObjectTracker(SAM2Base):
    ...
    def prepare_memory_conditioned_features(self,
        ...
        if self.past_frames['short_term'] or self.past_frames['long_term']:
            ...
            for t_pos, prev in t_pos_and_prevs:
                ...
                to_cat_memory.append(feats.flatten(2).permute(1, 0, 2))

                # Spatial positional encoding (it might have been offloaded to CPU in eval)
                maskmem_enc = prev["maskmem_pos_enc"][-1].to(device, non_blocking=True)

                maskmem_enc = maskmem_enc.flatten(2).permute(1, 0, 2)
    ...
    def reset_tracker_state(self):
        """
        Reset the tracker state by clearing the memory bank and resetting the current object index,
        Kalman filter states, and stable frame counters.
        """
        self.curr_obj_idx = 0
        self.past_frames = {'short_term': deque(maxlen=7), 'long_term': deque(maxlen=7)}
        self.kf_mean.clear()
